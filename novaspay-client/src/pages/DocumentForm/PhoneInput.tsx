...
const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  return (
    <div className={`flex flex-col w-full mt-2 ${className ?? ''}`}>
      ...
      <div className="flex overflow-hidden rounded-md border-none bg-white text-gray-500">
        {/* Country Code Selector */}
        <div className="flex w-28 items-center gap-1 px-3 py-2 border-none bg-gray-200 text-gray-500">
          <select
            value={countryCode}
            onChange={(e) => setCountryCode(e.target.value)}
            className="bg-transparent w-full h-full  outline-none text-sm appearance-none cursor-pointer"
          >
        ...
        <input
          ...
          className="flex-1 px-2 py-3 outline-none text-gray-500 text-sm rounded-r-md"
