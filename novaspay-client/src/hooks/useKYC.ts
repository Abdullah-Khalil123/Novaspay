import { createClientKYC, getClientKYC, updateClientKYC } from '@/actions/kyc';
import { queryClient } from '@/providers/react-query';
import type { Params } from '@/types/params';
import { useMutation, useQuery } from '@tanstack/react-query';

const useKYCs = (param?: Params, active?: boolean) => {
  return useQuery({
...
const useCreateClientKYC = () => {
  return useMutation({
    ...
    mutationFn: async (kycData: any) => createClientKYC(kycData),
    onSuccess: () => {
...
const useUpdateClientKYC = () => {
  return useMutation({
    mutationKey: ['updateClientKYC'],
    mutationFn: async (kycData: any) => updateClientKYC(kycData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['clientKYC'] });
    },
  });
...
export { useKYCs, useCreateClientKYC, useUpdateClientKYC };
